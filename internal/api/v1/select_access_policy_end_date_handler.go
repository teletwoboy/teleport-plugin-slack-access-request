...
func (i *InteractionHandler) HandleAccessPolicyEndDateSelection(payloadStr string, w http.ResponseWriter) {
	payload, err := blockactions.ParseAccessPolicyEndDateSelect(payloadStr)
	if err != nil {
	...
	builder := modal.NewSelectEndTimeBuilder(payload)
