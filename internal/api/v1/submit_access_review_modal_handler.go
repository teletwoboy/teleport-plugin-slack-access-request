...
func (i *InteractionHandler) HandleAccessReviewModalSubmission(payloadStr string, w http.ResponseWriter) {
	...
	// 2. 검증
	i.verifyAccessReviewModal(ctx, w, payload)
	...
	err = i.Services.Teleport.SubmitAccessRequestState(ctx, updateBuilder)
	if err != nil {
	...
	// 6. access_requests 테이블 row 업데이트 하기
	...
	// 9. 메시지에 띄울 requester 정보 가져오기
	requesterSlackUser, err := i.Services.Slack.GetUserBySlackUserID(ctx, updatedAccessRequest.RequesterUserID)
	if err != nil {
		res.ErrorMessageToSlack(i.Services.Slack, payload.ReviewerChannelID, err, w)
		return
	}

	// 10. Reviewer 에게 처리되었음을 알림
	builder := message.NewAccessReviewSubmissionBuilder(updatedAccessRequest, createdAccessReview, requesterSlackUser, slackUser)
	_, _, err = i.Services.Slack.PostMessage(payload.ReviewerChannelID, builder)
	if err != nil {
		res.ErrorMessageToSlack(i.Services.Slack, payload.ReviewerChannelID, err, w)
		return
	}

	// 11. Requestor 에게 처리되었음을 알림
	builder = message.NewAccessReviewToRequestorBuilder(accessRequest, accessReview, requesterSlackUser, slackUser)
	_, _, err = i.Services.Slack.PostMessage(updatedAccessRequest.InputChannelID, builder)
	if err != nil {
		res.ErrorMessageToSlack(i.Services.Slack, payload.ReviewerChannelID, err, w)
		return
	}
...
func (i *InteractionHandler) verifyAccessReviewModal(ctx context.Context, w http.ResponseWriter, payload *viewsubmission.AccessReviewModal) {
