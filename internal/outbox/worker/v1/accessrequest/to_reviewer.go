...
func (h *Handler) HandleToReviewerOutbox(ctx context.Context, ob *model.Outbox) error {
	ctx, cancel := context.WithTimeout(ctx, constant.ProcessingTimeout)
	defer cancel()

	ctx, span := tracer.Start(ctx, telemetry.WorkerAccessRequestToReviewer)
	defer span.End()

	// payload 역직렬화
	var payload accessrequest.ToReviewerPayload
	if err := json.Unmarshal([]byte(ob.Payload), &payload); err != nil {
		return err
	}
	reviewerChannelID := payload.ReviewerChannelID
	...
	// 메시지 전송하기
	builder := message.NewAccessRequestToReviewersBuilder(aRequest, slackUser)
	_, _, err = h.Services.Slack.PostMessageContext(ctx, reviewerChannelID, builder)
	if err != nil {
		return err
	}
