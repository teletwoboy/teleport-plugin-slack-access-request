...
type AccessPolicyStartDateSelectPayload struct {
...
type AccessPolicyStartDateSelectPrivateMetadataPayload struct {
...
type AccessPolicyStartDateSelect struct {
...
func ParseAccessPolicyStartDateSelect(payloadStr string) (*AccessPolicyStartDateSelect, error) {
	var payload AccessPolicyStartDateSelectPayload
	if err := json.Unmarshal([]byte(payloadStr), &payload); err != nil {
		return nil, fmt.Errorf("invalid payload format: %s", err)
	}

	strPrivateMetadata := payload.View.PrivateMetadata
	var privateMetadata AccessPolicyStartDateSelectPrivateMetadataPayload
	if err := json.Unmarshal([]byte(strPrivateMetadata), &privateMetadata); err != nil {
		return nil, fmt.Errorf("invalid payload format: %s", err)
	}
	return &AccessPolicyStartDateSelect{
