...
type fifthStepBuilder struct {
	payload *accessrequest.AccessDurationTimeSelect
	ttl     time.Time
}

func NewFifthStepBuilder(p *accessrequest.AccessDurationTimeSelect, t time.Time) modal.Builder {
...
func (f *fifthStepBuilder) BuildStartDateBlock() []slack.Block {
...
func (f *fifthStepBuilder) BuildAccessDurationBlock() []slack.Block {
...
func (f *fifthStepBuilder) BuildRequestTTLBlock() []slack.Block {
	text := BuildRequestTTLInfoText(f.ttl)
	requestTTLInfoBlock := slack.NewSectionBlock(
		slack.NewTextBlockObject(util.Markdown, text, false, false),
		nil,
		nil,
	)
	requestTTLOpts := f.BuildRequestTTLOpts()
	requestTTLOptsBlock := slack.NewActionBlock(
		util.ARequestRequestTTLOptionActionBlockID,
		slack.NewOptionsSelectBlockElement(
			util.StaticSelect,
			slack.NewTextBlockObject(util.PlainText, util.SelectOne, false, false),
			util.ARequestRequestTTLOptionBlockActionID,
			requestTTLOpts...,
		),
	)
	return []slack.Block{requestTTLInfoBlock, requestTTLOptsBlock}
}

func (f *fifthStepBuilder) BuildRequestTTLOpts() []*slack.OptionBlockObject {
	var requestTTLOpts []*slack.OptionBlockObject
	requestTTLOpts = append(requestTTLOpts,
		slack.NewOptionBlockObject(
			util.ARequestRequestTTLFirstOption,
			slack.NewTextBlockObject(util.PlainText, util.ARequestRequestTTLFirstOption, false, false),
			nil,
		),
		slack.NewOptionBlockObject(
			util.ARequestRequestTTLSecondOption,
			slack.NewTextBlockObject(util.PlainText, util.ARequestRequestTTLSecondOption, false, false),
			nil,
		),
	)
	return requestTTLOpts
}

func (f *fifthStepBuilder) BuildPrivateMetadata() (string, error) {
	privateMetadata := &accessrequest.RequestTTLOptionSelectPrivateMetadataPayload{
		...
		SelectedAccessDurationTime:       f.payload.AccessDurationTime,
		RequestTTL:                       f.ttl,
	}
